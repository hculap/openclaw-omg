/**
 * Quick eval script: test graph traversal engine on real registry data.
 * Run with: pnpm tsx tests/eval/graph-real-data.ts
 */
...
console.log('=== REAL GRAPH ANALYSIS ===\n')
...
for (const [, entry] of entries) {
  ...
  if (links.length > 0) {
    ...
    resolvedLinks += links.filter((l) => allIds.has(l)).length
  }
}
...
// --- Backlinks: which nodes are most referenced? ---
console.log('\n--- Most back-linked nodes (most referenced by others) ---')
...
for (const [id, count] of topBacklinked) {
  ...
  console.log(`  ${id} (${entry?.type}): ${count} backlinks — "${entry?.description?.slice(0, 60)}"`)
}
...
// --- Find paths between two interesting nodes ---
if (topBacklinked.length >= 2) {
...
clearGraphCache()
const t0 = performance.now()
getNeighbors(OMG_ROOT, entries, testNodeId ?? entries[0]![0], 'both', 2)
...
// Expansion simulation: top-10 keyword candidates + graph neighbors
