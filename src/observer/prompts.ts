...
// ---------------------------------------------------------------------------
// Extract prompt params type
// ---------------------------------------------------------------------------

/** All runtime parameters needed to build the Extract user prompt. */
export type ExtractUserPromptParams = ObserverUserPromptParams

// ---------------------------------------------------------------------------
// Extract system prompt (Phase 5 — split observer)
// ---------------------------------------------------------------------------

/**
 * Returns the system prompt for the Extract phase LLM call.
 *
 * Same node-type guide and priority rules as the legacy Observer prompt,
 * but replaces `<now-update>` with a structured `<now-patch>` element.
 *
 * The now-patch schema:
 *   <now-patch>
 *     <focus>Single sentence, max 200 chars</focus>
 *     <open-loops>comma-separated loop items</open-loops>
 *     <suggested-links>comma-separated canonical keys</suggested-links>
 *   </now-patch>
 */
...
to upsert (create or update), and how to patch the current-state snapshot.
...
- **now**        — (Produced via <now-patch>, not as an operation. Do not use as a type.)
...
Updated project description with new details from this session.
  ...
  <!-- Optional: patch the current-state snapshot. Omit if no meaningful change. -->
  <now-patch>
    <focus>Working on auth module — single sentence, max 200 chars</focus>
    <open-loops>JWT middleware, login tests, email verification</open-loops>
    <suggested-links>preferences.editor_theme, projects.my_app</suggested-links>
  </now-patch>
</observations>
\`\`\`
...
/**
 * Builds the per-turn user prompt for the Extract LLM call.
 * Same structure as the legacy Observer user prompt.
 */
export function buildExtractUserPrompt(params: ExtractUserPromptParams): string {
  return buildObserverUserPrompt(params)
}

// ---------------------------------------------------------------------------
// System prompt (legacy — use buildExtractSystemPrompt for new code)
// ---------------------------------------------------------------------------
 ...
 * @deprecated Use {@link buildExtractSystemPrompt} for new code.
 */
 ...
 * @deprecated Use {@link buildExtractUserPrompt} for new code.
 */
